    ...
    @if (count($empSched) >= 1)
        <span style="font-size: 11px;color:gray">
            @if (!$empSched[0]->second_in && !$empSched[0]->second_out)
                {{ date('G', strtotime($empSched[0]->first_in)) }} - {{ date('G', strtotime($empSched[0]->first_out)) }}
            @else
                {{ date('G', strtotime($empSched[0]->first_in)) }} - {{ date('G', strtotime($empSched[0]->first_out)) }} | {{ date('G', strtotime($empSched[0]->second_in)) }} - {{ date('G', strtotime($empSched[0]->second_out)) }}
            @endif
        </span>
        <script>
