    ...
    public function handle()
    {
        ...
        foreach ($employees as $employee) {
                ...
                if (Carbon::now()->isSameDay($twelve_months_after_hire) || Carbon::now()->gt($twelve_months_after_hire)) {
                    ...
                    $spl_given = EmployeeLeaveCreditLogs::whereHas('employeeLeaveCredit', function ($query) use ($employee, $twelve_months_after_hire) {
                        $query->where('employee_profile_id', $employee->id);
                    })
                    ->whereYear('created_at', $currentYear)// Check within the current 6-month interval
                    ->where('reason', 'Annual Solo Parent Credits')
                    ->exists();
