...
class MigratePISubDetailsController extends Controller
{
    ...
    public function create()
    {
        ...
        try {
            ...
            // DB::table('childs')->where('id', '!=', 1)->delete();
            // DB::table('identification_numbers')->where('id', '!=', 1)->delete();
            // DB::table('family_backgrounds')->where('id', '!=', 1)->delete();
            // DB::table('civil_service_eligibilities')->where('id', '!=', 1)->delete();
            // DB::table('educational_backgrounds')->where('id', '!=', 1)->delete();
            // DB::table('legal_information_questions')->where('id', '!=', 1)->delete();
            // DB::table('legal_informations')->where('id', '!=', 1)->delete();
            // DB::table('references')->where('id', '!=', 1)->delete();
            // DB::table('trainings')->where('id', '!=', 1)->delete();
            ...
            foreach ($csvData as $index => $row) {
                ...
                if (!$row['email address'] == '' || !$row['email address'] === null) {
                    ...
                    $personalId = EmployeeProfile::select('pi.*')->leftJoin('personal_informations as pi', 'employee_profiles.personal_information_id', 'pi.id')
                        ...
                        ->first();
                    if ($personalId->id == null || !isset($personalId->id)) {
                        continue;
                    }
                    // dd($personalId->id->id);
                    //Create Contact
                    ...
                    $contact->personal_information_id = $personalId->id;
                    $contact->save();
                } else {
                    $personalId = EmployeeProfile::select('pi.*')->leftJoin('personal_informations as pi', 'employee_profiles.personal_information_id', 'pi.id')
                        ->where('employee_profiles.employee_id', $row['id'])
            ...
            //             $child->birthdate = $val->birthdate == null ? Carbon::now() : $val->birthdate;
            //             $child->save();
            //         }
            //     }
