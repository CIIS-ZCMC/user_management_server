    ...
    public function store(EmployeeProfileNewResource $request)
    {
        try {
            ...
            if (strip_tags($request->shifting) === 0) {
                $schedule_this_month = Helpers::generateSchedule(Carbon::now(), $employee_profile->id, $request->meridian);
                ...
                $schedule_next_month = Helpers::generateSchedule(Carbon::now()->addMonth()->startOfMonth(), $employee_profile->id, $request->meridian);
