...
class AttendanceReportController extends Controller
{
     ...
     * @return void
     */
    private function sortData(&$profiles, $sort_by, $order)
    {
        usort($profiles, function ($a, $b) use ($sort_by, $order) {
     ...
     */
    public function xfilterAttendanceReport(Request $request)
    {
        try {
            $arr_data = [];
            ...
            // TEST AREA
            $divisionId = 12; // Example division ID

            $employees = AssignArea::with('employeeProfile.personalInformation')
                ->where('section_id', $divisionId)
                // ->orWhereIn('department_id', function ($query) use ($divisionId) {
                //     $query->select('id')->from('departments')->where('section_id', $divisionId);
                // })
                ->orWhereIn('section_id', function ($query) use ($divisionId) {
                    $query->select('id')->from('sections')->where('section_id', $divisionId);
                })
                ->orWhereIn('unit_id', function ($query) use ($divisionId) {
                    $query->select('id')->from('units')->whereIn('section_id', function ($subQuery) use ($divisionId) {
                        $subQuery->select('id')->from('sections')->where('section_id', $divisionId);
                    });
                })
                ->get();
